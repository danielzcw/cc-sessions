...
const upsertSession = db.prepare(`
  insert into sessions (
    session_id, project_dir, cwd, cwd_key, title, title_source, first_prompt,
...
const insertFts = db.prepare('insert into msg_fts (body, session_id, role, ts) values (?,?,?,?)')
const getStamp = db.prepare('select file_mtime, file_size from sessions where session_id = ?')

export function needsReindex(sessionId: string, mtimeMs: number, size: number): boolean {
  const row = getStamp.get(sessionId) as { file_mtime: number; file_size: number } | undefined
  if (!row) return true
  return row.file_mtime !== Math.floor(mtimeMs) || row.file_size !== size
}
...
export function saveSession(row: SessionRow, ftsRows: { body: string; role: string; ts: string | null }[]): void {
  ...
  try {
    upsertSession.run(
      row.session_id, row.project_dir, row.cwd, row.cwd_key, row.title, row.title_source,
    ...
    for (const f of ftsRows) {
      if (f.body.trim()) insertFts.run(f.body.slice(0, 20000), row.session_id, f.role, f.ts)
    }
    db.exec('commit')
  } catch (e) {
    db.exec('rollback')
...
export function allSessions(cwdKey?: string): SessionRow[] {
  const sql = cwdKey
    ? 'select * from sessions where cwd_key = ? order by coalesce(updated_at, created_at) desc'
    : 'select * from sessions order by coalesce(updated_at, created_at) desc'
  return (cwdKey ? db.prepare(sql).all(cwdKey) : db.prepare(sql).all()) as unknown as SessionRow[]
}

export function getSessionRow(sessionId: string): SessionRow | undefined {
  return db.prepare('select * from sessions where session_id = ?').get(sessionId) as unknown as SessionRow | undefined
}

export function projectRows(): { cwd: string; cwd_key: string; n: number; last: string | null; cost: number; dirs: string; branches: string }[] {
  return db.prepare(`
    select cwd_key,
           min(cwd) as cwd,
           count(*) as n,
    ...
    from sessions group by cwd_key order by last desc
  `).all() as unknown as { cwd: string; cwd_key: string; n: number; last: string | null; cost: number; dirs: string; branches: string }[]
}
...
export function search(q: string, opts: { cwdKey?: string; limit?: number } = {}): SearchHit[] {
  const query = q.trim()
  if (!query) return []
  ...
  const rows = cjkShort || query.length < 3
    ...
    ? db.prepare(`
        select f.session_id, f.role, f.ts, f.body, s.cwd, s.title
        from msg_fts f join sessions s on s.session_id = f.session_id
        where f.body like ? ${opts.cwdKey ? 'and s.cwd_key = ?' : ''}
        order by f.ts desc limit ?
      `).all(...[`%${query}%`, ...(opts.cwdKey ? [opts.cwdKey] : []), limit])
    : db.prepare(`
        select f.session_id, f.role, f.ts,
               snippet(msg_fts, 0, char(1), char(2), '…', 12) as body,
               s.cwd, s.title
        from msg_fts f join sessions s on s.session_id = f.session_id
        where msg_fts match ? ${opts.cwdKey ? 'and s.cwd_key = ?' : ''}
        order by rank limit ?
      `).all(...[ftsQuote(query), ...(opts.cwdKey ? [opts.cwdKey] : []), limit])

  return (rows as unknown as { session_id: string; role: string; ts: string | null; body: string; cwd: string; title: string }[])
    .map((r) => ({
...
export function stats(): StatsResponse {
  const tot = db.prepare('select sum(cost_usd) c, count(*) n, sum(msg_count) m from sessions').get() as
